...
def loaddb(curs, table, datafile, conn=None, verbose=True):
    ...
    for recstr in rows:
        curs.exeute('insert into ' + table + ' values ' + recstr)
    if conn: conn.commit()
...
if __name__ == '__main__':
    ...
    if len(sysy.argv) > 1: dbfile   = sys.argv[1]
    if len(sysy.argv) > 2: datafile = sys.argv[2]
    if len(sysy.argv) > 3: table    = sys.argv[3]
